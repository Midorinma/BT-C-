...
// Ð?nh ngh?a cho ki?u "struct City" 1 tên m?i ng?n g?n hõn, thay v? khai báo ki?u "struct City" th? ta ch? c?n dùng "City"
...
struct LinkedList{
 ...
 // Ð?nh ngh?a cho ki?u "struct LinkedList" 1 tên m?i ng?n g?n hõn, thay v? khai báo ki?u "struct LinkedList" th? ta ch? c?n dùng "Node"
...
Node addHead(Node head, City value){
 ...
 // Thêm vào ? "ch? s?" (b?t ð?u t? 0) b?t k?, n?u mu?n thêm theo "v? trí" (b?t ð?u t? 1) th? gi?m position ði 1 ðõn v?
...
Node delTail(Node head){
 ...
 // Xóa Node ? "ch? s?" (b?t ð?u t? 0) b?t k?
Node delAt(Node head, int position){
    if(position == 0 || head == NULL || head->next == NULL){
        head = delHead(head); // N?u v? trí xóa là 0, t?c là thêm vào ð?u
    }else{
        // B?t ð?u t?m v? trí c?n xóa. Ta s? dùng k ð? ð?m cho v? trí
        int k = 1;
        Node p = head;
...
// Hàm kh?i t?o Node ð?u tiên c?a LinkedList
Node initHead(){
    Node head;
    head = NULL;
    return head;
...
// Hàm tách các thành ph?n c?a 1 d?ng trong file
...
// Hàm t?m ch? s? c?a Node có d? li?u thành ph? mà m? code c?a nó trùng v?i giá tr? c?n t?m
...
// Hàm t?m ch? s? c?a Node có di?n tích l?n nh?t (gi? s? ch? có 1)
// N?u d? li?u có nhi?u hõn 1, chúng ta t?m max r?i duy?t l?i 1 l?n n?a ð? t?m ra các Node có giá tr? = max ðó
...
// Hàm t?m Node có dân s? l?n nh?t
...
// N?u s?p x?p tãng d?n th? desc = 0, gi?m d?n th? desc = 1
